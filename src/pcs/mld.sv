...
// 简化说明（记 TODO）：AM 的 BIP3/BIP7 字段置常值（0x00/0xFF），不做
//   BIP 误码统计；Clause 74 FEC 与 MLD 叠加未实现（MLD 模式 FEC 须关）。
// -----------------------------------------------------------------------------
...
// {M0,M1,M2,BIP3,M4,M5,M6,BIP7}，M4..M6 = ~M0..~M2；sync 头为控制型。
// BIP 字段本实现置常值（见文件头 TODO）。
function automatic logic [63:0] mld_am_payload(int lane);
  logic [7:0] m0, m1, m2;
  case (lane)
  ...
  endcase
  return {8'hFF, ~m2, ~m1, ~m0, 8'h00, m2, m1, m0};
endfunction
...
class mld_tx_c;
  ...
  protected int blk_cnt[MLD_MAX_LANES];   // 各 lane 距上个 AM 的块计数
  ...
  // 约定：各 lane 计数归零位置插 AM —— 各 lane 的 AM 在同一轮转轮次
  // 产生，接收端以此为对齐锚。
  function void push_block(block66_t b, output int lane,
    ...
    if (blk_cnt[lane] == 0) begin
      ...
      am_blk.payload = mld_am_payload(lane);
    end
